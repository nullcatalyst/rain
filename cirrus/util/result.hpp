...
class Result {
  ...
  public:
    ...
    [[nodiscard]] constexpr Result(const Result<T2>& other) noexcept {
        static_assert(std::is_constructible_v<T, T2>, "cannot cast to unrelated Result type");
    ...
    [[nodiscard]] static constexpr Result ok(const T& value) {
        static_assert(std::is_move_constructible_v<T>, "the result type is not copyable");
