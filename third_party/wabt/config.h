...
#define WABT_VERSION_STRING "@WABT_VERSION_STRING@"
...
// #define HAVE_WIN32_VT100

/* Whether the target architecture is big endian */
// #define WABT_BIG_ENDIAN
...
#if _M_IX86
inline unsigned long LowDword(unsigned __int64 value) { return (unsigned long)value; }
...
inline int Clz(unsigned int mask) { return Clz((unsigned long)mask); }
...
inline int Clz(unsigned __int64 mask) {
    ...
    if (result == 32) result += Clz(LowDword(mask));
...
inline int Ctz(unsigned int mask) { return Ctz((unsigned long)mask); }
...
inline int Ctz(unsigned __int64 mask) {
    ...
    if (result == 32) result += Ctz(HighDword(mask));
...
inline int Popcount(uint8_t value) { return Popcount((unsigned long)value); }

inline int Popcount(unsigned int value) { return Popcount((unsigned long)value); }
