...
llvm::Type* compile_struct_type(Context& ctx, ast::StructType& struct_type) {
    ...
    for (const auto& field : struct_type.fields()) {
        llvm_field_types.push_back(get_or_compile_type(ctx, *field.type));
    }

    // All structs are currently named. This may change in the future.
    // llvm::Type* llvm_type =
    ...
    llvm::Type* llvm_type = llvm::StructType::create(ctx.llvm_context(), llvm_field_types,
                                                     struct_type.name(), /*packed*/ false);
